...
- **Revision identity:** first-class and immutable per imported repository revision. Repeated imports are grouped into an owner-scoped lineage and browsable through `GET /repositories/{id}/lineage` and the repository detail page (RFC-0002). The versioned owner-scoped query API reads and traverses **one** sealed snapshot — symbols, neighbours, references, paths, evidence, and single-snapshot impact reachability. What is **not** implemented: any `Snapshot A ↔ Snapshot B` graph diff, cross-revision entity correspondence (rename/move recovery), structural delta classification, and product re-analysis orchestration driven from a lineage.
