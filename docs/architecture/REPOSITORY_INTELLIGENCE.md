...
- **Evidence navigation (#95):** each citation in the authentication explanation links to the existing repository Explorer (`/repositories/{id}?tab=Explorer&path=...&startLine=...&endLine=...&snapshotId=...`), which selects the cited file and highlights the exact line span in the same Monaco-based `CodePreview` used elsewhere in the product. No second source viewer was built.
