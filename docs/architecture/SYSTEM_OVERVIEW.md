...
| Area | Responsibility |
| --- | --- |
| `src/app/` | Shell, router, pages, and global stores. Every application route sits behind the `RequireAuth` guard. Exactly four routes are public: `/` (landing, which hosts the waitlist modal), `/login`, `/register`, and `/oauth/complete` — the provider redirect target, which must be reachable before the tab holds any access token. |
| `src/features/` | Domain features with colocated hooks, components, and stores: repositories, upload, explorer, architecture, dependencies, review, documentation, AI workspace, insights, auth, settings, plus the public `landing` and `waitlist` surfaces that sit outside the auth guard. |
...
**The public surface, stated exactly.** Three groups of routes are reachable without a token, deliberately: the session-establishing `/auth` routes (`register`, `login`, `refresh`), the OAuth start/callback pair, and **`POST /waitlist`** — the only public *write* route in the API. It exists because the landing page must accept a signup before anyone has an account or an invite. It writes one row to `waitlist_entries`, returns an identical response for a new signup, a repeat signup, and a concurrent collision (so membership of the list is never disclosed), and its abuse guard is a per-minute budget in the `auth` rate-limit class rather than authentication.

**OAuth sign-in.** Google and GitHub sign-in and account linking live under `/auth/oauth` and are inert unless provider credentials are configured — `GET /auth/oauth/providers` returns only providers with a real client id and secret, so an unconfigured deployment offers none. OAuth authenticates an *existing* account; it never creates one. A provider identity that matches no account is returned to the invite-gated registration flow, because `AuthService.register()` is the only account-creating path in the backend and it enforces the `approved_emails` allowlist.
