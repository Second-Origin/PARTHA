...
def test_zip_upload_persists_repository_and_analysis_completes(client):
    response = _upload(
        client,
        "sample.zip",
        _zip_bytes(
    ...
    status_response = client.get(f"/analysis/{repository['id']}/status")
...
def test_tar_gz_upload_is_supported(client):
    response = _upload(
        client,
...
def test_invalid_archive_returns_backend_validation_error(client):
    response = _upload(client, "broken.zip", b"not an archive")
...
def test_empty_archive_is_rejected(client):
    response = _upload(client, "empty.zip", _zip_bytes({}))
...
def test_duplicate_upload_name_returns_conflict(client):
    content = _zip_bytes({"repo/package.json": "{}"})

    first = _upload(client, "repo.zip", content)
...
def test_github_import_uses_backend_validation_and_duplicate_detection(client, monkeypatch: pytest.MonkeyPatch):
    ...
    first = client.post("/repositories/github", json={"url": "https://github.com/example/demo"})
    duplicate = client.post("/repositories/github", json={"url": "https://github.com/example/demo"})
    malformed_branch = client.post(
        "/repositories/github",
