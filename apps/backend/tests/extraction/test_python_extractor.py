...
def test_imports_become_observations_with_referent_text():
    ...
    imports = sorted((o.referent_text for o in result.observations if o.observed_kind == "import"))
...
def test_relative_imports_preserve_level_in_referent_text():
    ...
    imports = [o.referent_text for o in result.observations if o.observed_kind == "import"]
...
def test_function_local_import_is_not_exposed_as_a_file_wide_binding():
    ...
    assert [observation for observation in result.observations if observation.observed_kind == "import_binding"] == []
    assert [
        observation.referent_text for observation in result.observations if observation.observed_kind == "call_shadowed"
    ] == ["issue_token"]
