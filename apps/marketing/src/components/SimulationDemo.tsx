...
export function SimulationDemo() {
  ...
  const run = () => {
    ...
    SIMULATION_STEPS.forEach((_, index) => {
      const timer = setTimeout(() => setStepIndex(index + 1), 550 * (index + 1));
      timers.current.push(timer);
    });
    const finish = setTimeout(() => setPhase('done'), 550 * SIMULATION_STEPS.length + 300);
  ...
  return (
    <section id="simulation" className="mx-auto max-w-5xl px-6 py-20 sm:px-8">
      <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/30 bg-accent px-3 py-1 text-2xs font-semibold uppercase tracking-[0.14em] text-primary">
        Scripted simulation · sample repository
      </div>
      <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">See what a PARTHA analysis produces</h2>
      <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
        This is a scripted walkthrough of a made-up sample repository ({SAMPLE_REPO.name}), not a live analysis of any
        real code. It uses PARTHA's actual finding categories and output shape so you can see what a real run looks
        like without needing an account or a live backend.
      </p>

      <div className="partha-surface mt-8 overflow-hidden">
        <div className="flex items-center justify-between gap-4 border-b border-border bg-muted/50 px-5 py-4 sm:px-7">
        ...
        {phase === 'idle' && (
          <div className="flex flex-col items-center gap-4 px-6 py-16 text-center">
            <p className="max-w-sm text-sm text-muted-foreground">
              Run the simulation to watch PARTHA walk through a sample repository and produce Engineering Review and
              Repository Insights output.
            </p>
            <button
              type="button"
              onClick={run}
        ...
        {phase === 'done' && (
          <div className="animate-fade-in border-t border-border px-5 py-6 sm:px-7">
